...
MeetupRaffler.Helpers.getRandomMember = function(controller, previousWinners) {
	var memberIsValidForPrize = function(member_id, prize) {
		...
		if (previousWinners[prize]) {
			result = $.grep(previousWinners[prize], function(element, index) {
	...
	var rsvps = controller.get('model.rsvps');
	var min = 0;
	...
	if (selectedPrizeText !== "") {
		...
		controller.set('isSelectingPrizeWinner', true);
